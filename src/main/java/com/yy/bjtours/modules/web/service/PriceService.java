...
import com.yy.bjtours.modules.web.dao.PriceDao;
...
public class PriceService extends CrudService<PriceDao, TLinePriceDto> {
    private static final Logger logger = LoggerFactory.getLogger(PriceService.class);
    
    @Transactional(readOnly = false)
    public void updateLinePrice(TLinePriceDto tlpd) {
        if (tlpd == null) {
            logger.info("update_TLinePriceDto 对象不能为空");
            throw new RuntimeException("参数完整性校验失败！");
        }
        tlpd.setIsNewRecord(false);
        // 保存商户信息
        save(tlpd);
        logger.info("## method updateLinePrice 更新路线价钱信息功能");
    }
