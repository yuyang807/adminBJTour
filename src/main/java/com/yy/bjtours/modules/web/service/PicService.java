...
import com.yy.bjtours.modules.web.dao.LineDao;
import com.yy.bjtours.modules.web.dao.PicDao;
import com.yy.bjtours.modules.web.dto.TLineDto;
import com.yy.bjtours.modules.web.dto.TLineDtoVo;
...
public class PicService extends CrudService<PicDao, TPicDto> {
     ...
     * @return
     */
    public List<TPicDto> queryPicList(String urlFront){
    	return dao.findAllPicList(urlFront);
    }
     ...
     * @return
     */
    public List<TPicDto> queryPicLineByLineNo(long lineNo){
    	return dao.findPicByLineNo(lineNo);
    }
	
    @Transactional(readOnly = false)
    public void updateLine(TLineDto lineDto) {
        if (lineDto == null) {
            logger.info("update_lineDto 对象不能为空");
            throw new RuntimeException("参数完整性校验失败！");
        }
        lineDto.setIsNewRecord(false);
        // 保存商户信息
//        save(lineDto);
        logger.info("## method updateLine 更新路线信息功能");
    }
    
    @Transactional(readOnly = false)
    public int updatePic(TPicDto picDto){
    	return dao.updateByPicNo(picDto);
    }
