 ...
 */
package com.yy.bjtours.common.security.shiro.session;
...
import com.yy.bjtours.common.utils.JedisUtils;
				...
				if (StringUtils.isNotBlank(e.getKey()) && StringUtils.isNotBlank(e.getValue())){
					...
					if (ss != null && ss.length == 3){// jedis.exists(sessionKeyPrefix + e.getKey())){
						...
						// SESSION验证失败
							...
							logger.error("getActiveSessions {} ", session.getId(), e2);
...
//			}
    ...
    public Session readSession(Serializable sessionId) throws UnknownSessionException {
    	try{
			...
			logger.error("readSession {} ", sessionId, e);
			return null;
		}
    }
